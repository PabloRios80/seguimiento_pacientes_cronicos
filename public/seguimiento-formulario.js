...
function abrirModalResumen(data) {
  ...
  const alertas = data.evaluaciones.filter(
    (item) => item.calificacion === "Malo" || item.calificacion === "Regular" ||
  ...
  if (alertas.length === 0) {
    ...
    alertas.forEach((item) => {
      ...
      if (item.calificacion === "Malo") { colorClass = "bg-rose-50 border-rose-100"; textTitle = "text-rose-900"; badgeColor = "bg-rose-200 text-rose-900"; }
      else if (item.calificacion === "Regular") { colorClass = "bg-amber-50 border-amber-100"; textTitle = "text-amber-900"; badgeColor = "bg-amber-200 text-amber-900"; }
    ...
    json.historial.forEach((registro) => {
      ...
      item.innerHTML = `
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="font-bold text-slate-700 text-sm">${fStr}</p>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    showModalMessage("Error: Datos no encontrados.", true, () => (window.location.href = "/"));
    return;
  }
  ...
  [document.getElementById("close-estudios-modal"),
   document.getElementById("close-estudios-modal-bottom")].forEach((btn) => {
  ...
  verEstudiosBtn?.addEventListener("click", async () => {
    ...
    estudiosModalContent.innerHTML = '<p class="text-center text-gray-500 py-8"><i class="fas fa-spinner fa-spin mr-2"></i>Cargando estudios...</p>';
    estudiosModal.classList.remove("hidden");
    estudiosModal.classList.add("flex");

    try {
      const token = localStorage.getItem("dpToken");
      const res = await fetch("https://acceso.diapreventivoiapos.com/api/estudios-paciente", {
        method: "POST",
      ...
      if (!data.success || !data.estudios?.length) {
        estudiosModalContent.innerHTML = '<p class="text-center text-gray-400 py-8">No se encontraron estudios previos.</p>';
        return;
      }

      allEstudios = data.estudios;
      let html = '<div class="overflow-x-auto"><table class="min-w-full text-sm"><thead><tr class="bg-gray-50"><th class="py-2 px-4 border-b text-left">Tipo</th><th class="py-2 px-4 border-b text-left">Fecha</th><th class="py-2 px-4 border-b text-center">Acción</th></tr></thead><tbody>';
      ...
      estudiosModalContent.innerHTML = '<p class="text-center text-red-500 py-8">Error al cargar estudios.</p>';
    }
  });
  ...
  estudiosModalContent?.addEventListener("click", (e) => {
    ...
    if (estudio.Resultado) html += `<p class="mb-2"><strong>Resultado:</strong> ${estudio.Resultado}</p>`;
    if (estudio.Observaciones) html += `<p class="mb-2"><strong>Observaciones:</strong> ${estudio.Observaciones}</p>`;
    ...
    const links = estudio.LinksPDF?.filter((l) => l) || (estudio.LinkPDF ? [estudio.LinkPDF] : []);
    if (links.length) {
      html += '<div class="mt-4">';
      links.forEach((l, i) => {
