...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const removeFirebaseOverlay = () => {
    ...
    iframes.forEach((iframe) => {
      if (
        iframe.src &&
        (iframe.src.includes("firebase") || iframe.src.includes("google"))
      ) {
  ...
  const osteoporosisSectionDiv = document.getElementById(
    "osteoporosis-section",
  );
  const aspirinaSectionDiv = document.getElementById("aspirina-section");
  const estudiosComplementariosSeccion = document.getElementById(
    "estudios-complementarios-seccion",
  );
  const verEstudiosBtn = document.getElementById("ver-estudios-btn");
  const resultadosEstudiosPacienteDiv = document.getElementById(
    "resultados-estudios-paciente",
  );

  const labResultsModal = document.getElementById("lab-results-modal");
  const labResultsModalContent = document.getElementById(
    "lab-results-modal-content",
  );
  const closeLabResultsModal = document.getElementById(
    "close-lab-results-modal",
  );
  const generarInformeBtn = document.getElementById("generar-informe-btn");

  const enfermeriaResultsModal = document.getElementById(
    "enfermeria-results-modal",
  );
  const enfermeriaResultsModalContent = document.getElementById(
    "enfermeria-results-modal-content",
  );
  const closeEnfermeriaResultsModal = document.getElementById(
    "close-enfermeria-results-modal",
  );
  ...
  resultadosEstudiosPacienteDiv.addEventListener("click", (event) => {
    ...
    if (labBtn) {
      console.log(
        "DEBUG: Click en botón Laboratorio, índice:",
        labBtn.dataset.index,
      );
      ...
      if (!isNaN(index) && allFetchedStudies[index]) {
        ...
        if (labStudy.ResultadosLaboratorio) {
          ...
          console.error(
            'ERROR: El estudio de laboratorio no tiene la propiedad "ResultadosLaboratorio".',
          );
        }
      } else {
        console.error(
          "ERROR: No se pudo obtener un índice válido para el estudio de laboratorio clicado.",
        );
      }
    } else if (enfermeriaBtn) {
      console.log(
        "DEBUG: Click en botón Enfermería, índice:",
        enfermeriaBtn.dataset.index,
      );
      ...
      if (!isNaN(index) && allFetchedStudies[index]) {
        ...
        if (enfermeriaStudy.ResultadosEnfermeria) {
          ...
          console.log(
            "DEBUG: Abriendo modal de Enfermería con datos:",
            enfermeriaStudy.ResultadosEnfermeria,
          );
        } else {
          console.error(
            'ERROR: El estudio de enfermería no tiene la propiedad "ResultadosEnfermeria".',
          );
        }
      } else {
        console.error(
          "ERROR: No se pudo obtener un índice válido para el estudio de enfermería clicado.",
        );
      }
    }
  });
  ...
  if (
    !estudiosComplementariosSeccion ||
    !verEstudiosBtn ||
    !resultadosEstudiosPacienteDiv
  ) {
    console.warn(
      "Algunos elementos DOM para estudios complementarios no se encontraron. Asegúrate de que index.html los tenga.",
    );
  }
    ...
    if (generarInformeBtn) {
      generarInformeBtn.addEventListener("click", () => {
        ...
        if (currentPatientData) {
          console.log(
            "DEBUG: currentPatientData NO es nulo.",
            currentPatientData,
          );
          console.log("DEBUG: currentRedFlags antes de pasar:", [
            ...currentRedFlags,
          ]);
          ...
          alert(
            "Por favor, busca un paciente primero para generar un informe de seguimiento.",
          );
        }
      });
    }
  ...
  async function consultarDNI() {
    ...
    resultDiv.innerHTML =
      '<p class="text-center text-gray-500 py-8"><i class="fas fa-spinner fa-spin"></i> Buscando información...</p>';
    resultDiv.classList.remove("hidden");
    resultDiv.style.display = "block";

    const previousStudiesMessageDiv = document.getElementById(
    ...
    try {
      ...
      if (data.iapos?.esActivo) {
        ...
        if (data.alertas?.length > 0 && previousStudiesMessageDiv) {
          ...
          if (data.historialDP?.length > 1) {
            let html =
              '<p class="text-yellow-700 font-semibold mb-2"><i class="fas fa-history mr-2"></i>DPs anteriores:</p><ul class="list-disc list-inside ml-4">';
        ...
        if (estudiosComplementariosSeccion) {
          ...
          resultadosEstudiosPacienteDiv.innerHTML =
            '<p class="text-gray-600">Haz clic en "Ver Estudios" para cargar los informes complementarios.</p>';
        ...
        resultDiv.innerHTML =
          '<p class="text-center text-red-500 py-8">⚠️ El afiliado no está activo en IAPOS.</p>';
        resultDiv.classList.remove("hidden");
        resultDiv.style.display = "block";
        resetProfile();
      }
      ...
      resultDiv.innerHTML =
        '<p class="text-center text-red-500 py-8">Error al conectar con el servidor</p>';
      resultDiv.classList.remove("hidden");
      resultDiv.style.display = "block";
      resetProfile();
    } finally {
  ...
  function openLabResultsModal(results) {
    ...
    console.log(
      "DESPUÉS de remover hidden - Modal classList:",
      labResultsModal.classList,
    );
  }
  ...
  function openEnfermeriaResultsModal(results) {
    ...
    if (
      enfermeriaData.Espirometria_PDF &&
      enfermeriaData.Espirometria_PDF.trim() !== ""
    ) {
  ...
  if (verEstudiosBtn) {
    verEstudiosBtn.addEventListener("click", async () => {
      ...
      resultadosEstudiosPacienteDiv.innerHTML =
        '<p class="text-gray-600"><i class="fas fa-spinner fa-spin"></i> Cargando informes complementarios...</p>';
      ...
      try {
        ...
        console.log(
          "DEBUG (app.js - /obtener-estudios-paciente): Datos recibidos del Backend:",
          result,
        );
        if (response.ok && result.estudios && result.estudios.length > 0) {
          ...
          result.estudios.forEach((estudio, index) => {
            ...
            if (
              ...
              estudio.ResultadosLaboratorio &&
              typeof estudio.ResultadosLaboratorio === "object"
            ) {
              resultadoCeldaContent =
                '<p class="text-gray-600 text-sm">Ver detalles en tabla.</p>';
            ...
            // Se verifica si el tipo de estudio es Espirometria
              ...
              const esLink =
                link && String(link).trim() !== "" && link.startsWith("http");

              resultadoCeldaContent = resultado;
              informeCeldaContent = esLink
                ? `<a href="${link}" target="_blank" class="text-blue-500 hover:underline">Ver PDF <i class="fas fa-external-link-alt ml-1"></i></a>`
                : "<span>N/A</span>";
            } else {
              // Lógica para todos los demás estudios (Mamografia, Odontologia, etc.)
              resultadoCeldaContent = estudio.Resultado || "N/A";
              const linkPdf =
                estudio.LinkPDF && String(estudio.LinkPDF).trim() !== ""
                  ? estudio.LinkPDF
                  : null;
        ...
        console.error(
          "ERROR en frontend al buscar estudios (verEstudiosBtn):",
          error,
        );
        resultadosEstudiosPacienteDiv.innerHTML =
          '<p class="text-red-500">Error al buscar estudios. Intente de nuevo.</p>';
      } finally {
        verEstudiosBtn.disabled = false;
      }
    });
  ...
  document.addEventListener("click", function (event) {
    ...
    if (event.target.classList.contains("ver-lab-results-btn")) {
      ...
      if (estudio && estudio.ResultadosLaboratorio) {
        ...
        alert(
          "Error: No se encontraron datos de laboratorio para este estudio.",
        );
      }
    }
    ...
    if (event.target.classList.contains("ver-enfermeria-results-btn")) {
      ...
      if (estudio && estudio.ResultadosEnfermeria) {
        ...
        alert(
          "Error: No se encontraron datos de enfermería para este estudio.",
        );
      }
    }
  });
  ...
  function updateProfile(data) {
    ...
    document.getElementById("user-name").textContent =
      `${nombre} ${data.Apellido || data.apellido || ""}`.trim();
    document.getElementById("welcome-message").innerHTML =
  ...
  function showResults(data) {
    ...
    if (practicaSeleccionada) {
      ...
      const nombrePractica =
        practicaSelect.options[practicaSelect.selectedIndex].text;

      // Solo buscar observaciones si no es el campo "Profesional"
      let observaciones = "";
      if (
        practicaSeleccionada !== "Profesional" &&
        mapaObservaciones[practicaSeleccionada]
      ) {
  ...
  function evaluateCardiovascularRisk(data) {
    ...
    const valoresReales = {
      ...
      sexo: (data.Sexo || data.sexo || "").toUpperCase().startsWith("F")
        ...
        : "Masculino",
      presion:
        data["Presion Arterial"] || data["Presion_Arterial"] || "No registrado",
    ...
    const factoresEvaluados = {
      ...
      diabetes: {
        valor: valoresReales.diabetes,
        riesgo: valoresReales.diabetes.toLowerCase() === "presenta",
      }, // Convertir a minúsculas
      dislipemia: {
        valor: valoresReales.dislipemia,
        riesgo: valoresReales.dislipemia.toLowerCase() === "presenta",
      }, // Convertir a minúsculas
      ...
      sexo: {
        valor: valoresReales.sexo,
        riesgo: valoresReales.sexo === "Masculino",
      },
    };
    ...
    const riesgo = clasificarRiesgo(
      puntuacion,
      factoresEvaluados.diabetes.riesgo,
      factoresEvaluados.presion.riesgo,
    );
    ...
    if (factoresEvaluados.presion.riesgo) {
      ...
      valoresReales.presion.toLowerCase().includes("no se realiza") ||
      valoresReales.presion.toLowerCase().includes("no aplica")
    ) {
    ...
    if (factoresEvaluados.dislipemia.riesgo) {
      ...
      valoresReales.dislipemia.toLowerCase().includes("no se realiza") ||
      valoresReales.dislipemia.toLowerCase().includes("no aplica") ||
      valoresReales.dislipemia === "No registrado"
    ) {
    ...
    if (factoresEvaluados.diabetes.riesgo) {
      ...
      valoresReales.diabetes.toLowerCase().includes("no se realiza") ||
      valoresReales.diabetes.toLowerCase().includes("no aplica") ||
      valoresReales.diabetes === "No registrado"
    ) {
      currentRedFlags.add("Control Glucémico (Pendiente)");
    }

    // Tabaquismo
    if (factoresEvaluados.tabaquismo.riesgo) {
      // Si evaluarTabaquismo retorna riesgo=true
      currentRedFlags.add("Tabaquismo");
    } else if (
      valoresReales.tabaquismo.toLowerCase().includes("no se realiza") ||
      valoresReales.tabaquismo.toLowerCase().includes("no aplica") ||
      valoresReales.tabaquismo === "No registrado"
    ) {
    ...
    if (shouldAddIMCFlag) {
      ...
      console.log(
        "DEBUG IMC: IMC no es una categoría de riesgo/sobrepeso ni pendiente. No se añade bandera.",
      );
      // AÑADE ESTA LÍNEA AQUÍ para que el IMC pendiente también pase a seguimiento:
      if (
        pendingKeywords.includes(imcValueString) &&
        !currentRedFlags.has("IMC (Pendiente)")
      ) {
  ...
  function evaluarPresion(presion) {
    ...
    if (
      presionClean.includes("hipertensión") ||
      presionClean.includes("hipertension")
    ) {
      riesgo = true;
      ...
      presionClean.includes("no se realiza") ||
      presionClean.includes("no aplica")
    ) {
  ...
  function evaluarIMC(imc) {
    ...
    if (!isNaN(imcNum)) {
      ...
      imc.toLowerCase().includes("sobrepeso") ||
      imc.toLowerCase().includes("obesidad")
    ) {
      riesgo = true;
    }
  ...
  function calcularPuntuacionRiesgo(factores) {
    ...
    if (factores.edad.valor >= 70)
      puntos += factores.sexo.valor === "Masculino" ? 8 : 9;
    else if (factores.edad.valor >= 60)
      puntos += factores.sexo.valor === "Masculino" ? 6 : 7;
    else if (factores.edad.valor >= 50) puntos += 4;
    else if (factores.edad.valor >= 40)
      puntos += factores.sexo.valor === "Masculino" ? 3 : 2;
  ...
  function clasificarRiesgo(puntuacion, diabetes, hipertension) {
    ...
    if (puntuacion >= 15 || diabetes || hipertension) {
      return {
        ...
        clase: "risk-high",
        recomendacion:
          "Consulta cardiológica urgente. Control estricto de factores de riesgo.",
      };
  ...
  function actualizarEstiloTarjeta(tipo, tieneRiesgo) {
    ...
    if (!card || !notesElement || !valueElement) {
      console.warn(
        `Elementos de tarjeta, notas o valor no encontrados para ID: ${tipo}`,
      );
      return;
    }

    // Obtenemos el valor real del elemento y lo normalizamos para la comparación
    const actualValueClean = String(valueElement.textContent)
      .trim()
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
    ...
    if (PENDING_KEYWORDS_FOR_VISUALS.includes(actualValueClean)) {
      ...
      notesElement.innerHTML =
        '<span class="text-gray-500"><i class="fas fa-info-circle"></i> No realizado / Pendiente</span>';
      ...
      notesElement.innerHTML =
        '<span class="text-green-500"><i class="fas fa-check-circle"></i> No aplica</span>';
    } else if (tieneRiesgo) {
      // Si hay riesgo -> ROJO
      card.className = `p-4 rounded-lg bg-red-100 border-l-4 border-red-500`;
      notesElement.innerHTML =
        '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Riesgo detectado</span>';
    } else {
      // Si no es ninguno de los anteriores y no tiene riesgo -> VERDE (Normal)
      card.className = `p-4 rounded-lg bg-green-100 border-l-4 border-green-500`;
      notesElement.innerHTML =
        '<span class="text-green-500"><i class="fas fa-check-circle"></i> Normal</span>';
    }
  }
  ...
  function mostrarResultadosFinales({
    ...
    document.getElementById("pressure-value").textContent =
      valoresReales.presion;
    document.getElementById("imc-value").textContent = valoresReales.imc;
    document.getElementById("diabetes-value").textContent =
      valoresReales.diabetes;
    document.getElementById("dislipemia-value").textContent =
      valoresReales.dislipemia;
    document.getElementById("tabaquismo-value").textContent =
      valoresReales.tabaquismo;
    document.getElementById("edad-value").textContent =
      `${valoresReales.edad} años`;
    ...
    document.getElementById("risk-card").className =
      `md:col-span-2 p-4 rounded-lg ${riesgo.clase}`;
  ...
  function evaluateCancerPrevention(data) {
    ...
    for (const [testId, testInfo] of Object.entries(cancerTests)) {
      ...
      console.log(
        `CANCER_DEBUG: Procesando test: ${testInfo.name} (ID: ${testId})`,
      );
      console.log(
        `CANCER_DEBUG: Valor ORIGINAL para ${testInfo.name}: "${originalValue}"`,
      );
      ...
      console.log(
        `CANCER_DEBUG: Valor EVALUADO FINAL para ${testInfo.name} (minúsculas, sin acentos): "${evaluatedValue}"`,
      );
      // --- FIN NUEVOS LOGS ---
      ...
      const isPatologico =
        /patologico|patologica|alterado|alterada|positivo/i.test(
          evaluatedValue,
        ); // Añadido 'patologico' y 'alterada'
      const isNormal = /normal|negativo/i.test(evaluatedValue);
      const isNoRealizado = /no se realiza|no realizado|no aplica/i.test(
        evaluatedValue,
      ); // Añadido 'no aplica' en minúsculas
      ...
      if (isPatologico) {
        ...
        notesElement.innerHTML =
          '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Resultado patológico - Requiere atención</span>';
        ...
        notesElement.innerHTML =
          '<span class="text-green-500"><i class="fas fa-check-circle"></i> Resultado normal</span>';
      } else if (isNoRealizado) {
        cardElement.className = "p-4 rounded-lg bg-gray-100";
        notesElement.innerHTML =
          '<span class="text-gray-500"><i class="fas fa-info-circle"></i> No realizado</span>';
        ...
        if (
          (testInfo.gender === "Ambos" || testInfo.gender === sexo) &&
        ...
        cardElement.className = "p-4 rounded-lg bg-gray-100";
        notesElement.innerHTML =
          '<span class="text-gray-500"><i class="fas fa-question-circle"></i> No registrado / Valor no reconocido</span>'; // Mensaje más claro
  ...
  function evaluateInfectiousDiseases(data) {
    ...
    for (const [testId, testInfo] of Object.entries(infectiousTests)) {
      ...
      if (isPositive) {
        cardElement.className = "p-4 rounded-lg risk-high";
        notesElement.innerHTML =
          '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Resultado positivo - Requiere atención</span>';
        ...
        cardElement.className = "p-4 rounded-lg risk-low";
        notesElement.innerHTML =
          '<span class="text-green-500"><i class="fas fa-check-circle"></i> Resultado negativo</span>';
      } else if (isNotDone) {
        cardElement.className = "p-4 rounded-lg bg-gray-100";
        notesElement.innerHTML =
          '<span class="text-gray-500"><i class="fas fa-info-circle"></i> No realizado</span>';
        ...
        cardElement.className = "p-4 rounded-lg bg-gray-100";
        notesElement.innerHTML =
          '<span class="text-gray-500"><i class="fas fa-question-circle"></i> No registrado</span>';
  ...
  function evaluateHealthyHabits(data) {
    ...
    const healthyHabits = {
      alimentacion: {
        ...
        recommendationNegative:
          "Considera mejorar tu dieta con más frutas, verduras y alimentos integrales",
      ...
      actividad: {
        ...
        recommendationPositive:
          "Buen nivel de actividad física, mantén la rutina",
        recommendationNegative:
          "Intenta realizar al menos 150 minutos de actividad moderada por semana",
        recommendationNotDone:
          "Recomendación: Evaluación de actividad física anual",
      },
      seguridad: {
        ...
        recommendationNegative:
          "Recuerda usar siempre cinturón de seguridad y casco en motocicletas",
        recommendationNotDone:
          "Recomendación: Revisión de hábitos de seguridad vial",
      },
      caidas: {
        ...
        recommendationNegative:
          "Evalúa la seguridad en tu hogar y usa calzado adecuado",
      ...
      alcohol: {
        ...
        recommendationNotDone:
          "Recomendación: Evaluación de consumo de alcohol anual",
      },
      tabaco: {
        field: "Tabaco",
        ...
        recommendationPositive:
          "No fumas o has dejado de fumar, excelente decisión",
      ...
      acido: {
        ...
        recommendationNegative:
          "Considera suplementación si estás en edad fértil",
        recommendationNotDone:
          "Recomendación: Evaluación de niveles de ácido fólico",
      },
    };
    ...
    for (const [habitId, habitInfo] of Object.entries(healthyHabits)) {
      ...
      console.log(
        `DEBUG_COMPARISON: El valor final a comparar es: "${evaluatedValue}"`,
      );
      ...
      console.log(
        "DEBUG: Hábito: " +
          habitInfo.name +
          ', Valor EVALUADO (minúsculas): "' +
          evaluatedValue +
          '" (longitud: ' +
          evaluatedValue.length +
          ")",
      );
      // --- FIN LOGS DEPURACIÓN ---
      ...
      console.log(
        `DEBUG: ${habitInfo.name} - FINAL isNotDone: ${isNotDone}, isNegative: ${isNegative}, isPositive: ${isPositive}`,
      );
      // --- FIN LOGS DEPURACIÓN ---
      ...
      if (isNotDone) {
        // Si se marcó como "No realizado" / Pendiente (gris)
        ...
        // Si se marcó como Negativo / Requiere mejora (rojo)
        cardElement.className = "p-4 rounded-lg risk-high"; // Rojo para negativo
        notesElement.innerHTML =
          '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Requiere mejora</span>';
        ...
        // Si se marcó como Positivo (verde)
        cardElement.className = "p-4 rounded-lg risk-low"; // Verde para positivo
        notesElement.innerHTML =
          '<span class="text-green-500"><i class="fas fa-check-circle"></i> Adecuado</span>';
        ...
        // Si el valor no coincide con ninguna categoría conocida (ni negativo, ni pendiente, ni positivo)
        cardElement.className = "p-4 rounded-lg bg-gray-100"; // Color por defecto (gris)
        notesElement.innerHTML =
          '<span class="text-gray-500"><i class="fas fa-question-circle"></i> Dato no categorizado / No registrado</span>';
  ...
  function evaluateDentalHealth(data) {
    ...
    if (isHighRisk) {
      cardElement.className = "p-4 rounded-lg risk-high";
      notesElement.innerHTML =
        '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Riesgo Alto</span>';
      recommendationsList.innerHTML = `
      ...
      notesElement.innerHTML =
        '<span class="text-yellow-500"><i class="fas fa-exclamation-circle"></i> Riesgo Moderado</span>';
      recommendationsList.innerHTML = `
      ...
      cardElement.className = "p-4 rounded-lg risk-low";
      notesElement.innerHTML =
        '<span class="text-green-500"><i class="fas fa-check-circle"></i> Riesgo Bajo</span>';
      recommendationsList.innerHTML = `
  ...
  function evaluateMentalHealth(data) {
    ...
    document.getElementById("depresion-value").textContent =
      depresionValue || "No registrado";
    ...
    if (isDepresion) {
      ...
      depresionNotesElement.innerHTML =
        '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Se verifica</span>';
      recommendationsList.innerHTML += `
      ...
      depresionNotesElement.innerHTML =
        '<span class="text-green-500"><i class="fas fa-check-circle"></i> No se verifica</span>';
      recommendationsList.innerHTML += `
    ...
    document.getElementById("violencia-value").textContent =
      violenciaValue || "No registrado";
    ...
    if (isViolencia) {
      ...
      violenciaNotesElement.innerHTML =
        '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Se verifica</span>';
      recommendationsList.innerHTML += `
  ...
  function evaluateRenalHealth(data) {
    ...
    if (isPathological) {
      cardElement.className = "p-4 rounded-lg risk-high";
      notesElement.innerHTML =
        '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Resultado Patológico</span>';
      recommendationsList.innerHTML = `
      ...
      cardElement.className = "p-4 rounded-lg bg-gray-100";
      notesElement.innerHTML =
        '<span class="text-gray-500"><i class="fas fa-info-circle"></i> No se Realiza</span>';
      recommendationsList.innerHTML = `
      ...
      cardElement.className = "p-4 rounded-lg bg-gray-100";
      notesElement.innerHTML =
        '<span class="text-gray-500"><i class="fas fa-question-circle"></i> No se Realiza</span>';
  ...
  function evaluateAspirina(data) {
    ...
    // Mostrar valores
    document.getElementById("aspirina-value").textContent =
      value || "No registrado";
    ...
    if (isIndicated) {
      cardElement.className = "p-4 rounded-lg risk-high";
      notesElement.innerHTML =
        '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Indicada</span>';
      recommendationsList.innerHTML = `
      ...
      cardElement.className = "p-4 rounded-lg risk-low";
      notesElement.innerHTML =
        '<span class="text-green-500"><i class="fas fa-check-circle"></i> No indicada</span>';
      recommendationsList.innerHTML = `
  ...
  function evaluateVisualHealth(data) {
    ...
    // Mostrar valores
    document.getElementById("agudeza-value").textContent =
      value || "No registrado";
    ...
    if (isAlterada) {
      cardElement.className = "p-4 rounded-lg risk-high";
      notesElement.innerHTML =
        '<span class="text-red-500"><i class="fas fa-exclamation-triangle"></i> Alterada</span>';
      recommendationsList.innerHTML = `
  ...
  if (window.location.pathname === "/estadisticas.html") {
    ...
    const combinacionFiltrosSelect = document.getElementById(
      "combinacion-filtros",
    );
    ...
    if (
      ...
      filtrosAplicadosDiv &&
      consultarGrupoBtn &&
      resultadosResumenDiv &&
      exportarExcelBtn &&
      combinacionFiltrosSelect
    ) {
      cargarCampos();
    ...
    function agregarNuevoFiltro() {
      const camposSeleccionados = Array.from(
        selectorCampos.selectedOptions,
        ...
        try {
          ...
          if (opciones && opciones.length > 0) {
            opciones.forEach((opcion) => {
              ...
              labelOpcion.setAttribute(
                "for",
                `opcion-${campo}-${opcion.replace(/\s+/g, "-")}`,
              );
    ...
    async function realizarConsultaGrupal() {
      ...
      try {
        ...
        if (resultados && Array.isArray(resultados.data)) {
          ...
          resultadosResumenDiv.textContent =
            "Error: No se recibieron datos detallados para la exportación.";
          console.error(
            "Error: No se recibieron datos detallados para la exportación:",
            resultados,
          );
    ...
    function obtenerCriteriosDeFiltro() {
      const filtros = [];
      const filtrosDivs = document.querySelectorAll(
      ...
      filtrosDivs.forEach((filtroDiv) => {
        ...
        if (campo === "Edad") {
          ...
          const checkboxes = filtroDiv.querySelectorAll(
    ...
    function mostrarResultadosResumen(resultados) {
      ...
      if (
        !resultados ||
        typeof resultados.total_registros === "undefined" ||
        typeof resultados.conteo_cruce === "undefined"
      ) {
        resultadosResumenDiv.textContent =
          "Error al recibir los resultados del cruce.";
...
function continuarPeseMalBloqueo() {
  ...
  if (window._datosBloqueoActual) {
    document.dispatchEvent(
      new CustomEvent("cargarDPAnterior", {
